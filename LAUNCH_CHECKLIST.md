...
- 步骤：在习惯库页切换到“我的习惯”Tab
- 预期：列表正常加载，无 `云函数依赖缺失，请重新部署` 的提示
- 如出现提示：对 `getMyHabits` 执行“上传并部署：云端安装依赖”后重试

#### 支付演练小指南（可选）

- 前置配置：在云开发控制台绑定商户号/密钥；在环境变量添加 `SUB_MCH_ID=<子商户号>`。
- 部署函数：对 `createPayment` 与 `activateMembership` 执行“上传并部署：云端安装依赖”。
- 演练步骤：打开会员页 → 点击支付 → 支付成功后会员状态应变为“已激活”，云函数日志出现“订单创建/会员激活成功”。
- 故障排查：
  - 显示“待配置商户”：未配置商户或缺少 `SUB_MCH_ID` 环境变量。
  - 支付失败：查看云函数日志，核对商户密钥与子商户号；重新部署并重试。
