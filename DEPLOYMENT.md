...
10. `getMyHabits` - 获取“我的习惯”
11. `initTemplates` - 导入模板数据(用完可删)
...
1. 确认 `cloudfunctions/getMyHabits/package.json` 已包含依赖：`"wx-server-sdk": "~2.6.3"`
2. 部署时务必选择“云端安装依赖”，避免运行时缺库。
3. 部署后在云函数日志检查：无 `-504002` 或 `Cannot find module 'wx-server-sdk'` 报错。
4. 建议统一各云函数的 `wx-server-sdk` 版本至 `~2.6.3`，保持一致性。
...
- 在习惯库页切换到“我的习惯”Tab
- 期望：列表正常加载，无 `云函数依赖缺失，请重新部署` 的提示
- 如出现依赖缺失提示：重新对 `getMyHabits` 执行“上传并部署：云端安装依赖”，再重试
...
### 5. 依赖缺失（wx-server-sdk）

**症状**: 云函数报错 `-504002` 或日志显示 `Cannot find module 'wx-server-sdk'`

**排查步骤**:
1. 检查 `cloudfunctions/getMyHabits/package.json` 是否包含 `wx-server-sdk`
2. 部署流程是否选择了“云端安装依赖”
3. 在云函数日志确认依赖安装完成，无缺库报错
